...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(process.env.NEXT_PUBLIC_API_URL ? `${process.env.NEXT_PUBLIC_API_URL}/api/auth/login` : 'http://localhost:3001/api/auth/login', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-[hsl(var(--background))] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-foreground/60 text-sm">Enter your credentials to access your chats.</p>
        </div>
